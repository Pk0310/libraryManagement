...
const MemberCard = ({ member, onDeleteMember, onUpdateMember }) => {
  ...
  const fetchTransactions = async () => {
    try {
      const response = await axios.get('https://pratyasha1.pythonanywhere.com/api/transactions/');
      ...
      const transactionsWithBookNames = await Promise.all(memberTransactions.map(async transaction => {
        const bookResponse = await axios.get(`https://pratyasha1.pythonanywhere.com/api/books/${transaction.book}/`);
        return {
          ...transaction,
  ...
  const handleReturnBook = async (transactionId) => {
    ...
    try {
      const response = await axios.post('https://pratyasha1.pythonanywhere.com/api/return-book/', {
  ...
  return (
    <div className="bg-black rounded-lg shadow-md p-6 mb-4 text-white">
      ...
      <div className="mt-4 flex justify-end">
        {isEditing ? (
          ...
          <div>
            <button
              className="bg-pink-700 text-white py-2 px-4 rounded hover:bg-blue-900 mr-2"
                    ...
                    <tr key={transaction.id}>
                      ...
                      <td className="px-4 py-2">
                        {!transaction.return_date && (
                          <button
                            className="bg-gray-700 text-white py-1 px-2 rounded hover:bg-blue-900"
...
const MemberList = () => {
  ...
  const fetchMembers = async () => {
    try {
      const response = await axios.get('https://pratyasha1.pythonanywhere.com/api/members/');
      const membersWithIssuedBooks = await Promise.all(response.data.map(async member => {
        const issuedBooksResponse = await Promise.all(member.books_issued.map(async bookId => {
          const bookResponse = await axios.get(`https://pratyasha1.pythonanywhere.com/api/books/${bookId}/`);
  ...
  const deleteMember = async (memberId) => {
    try {
      await axios.delete(`https://pratyasha1.pythonanywhere.com/api/members/${memberId}/`);
      fetchMembers();
    } catch (error) {
  ...
  const updateMember = async (updatedMember) => {
    try {
      await axios.put(
        `https://pratyasha1.pythonanywhere.com/api/members/${updatedMember.id}/`,
        updatedMember
      );
      fetchMembers();
    } catch (error) {
  ...
  const addMember = async () => {
    try {
      await axios.post('https://pratyasha1.pythonanywhere.com/api/members/', {
        name: newMemberName,
    ...
    <div className="container mx-auto py-8">
      <div className="mb-4">
        ...
        <div className="flex">
          ...
          />
<button
  className="bg-yellow-400 text-white py-2 px-4 rounded hover:bg-yellow-500 ml-2"
  onClick={addMember}
>
  Add
</button>
