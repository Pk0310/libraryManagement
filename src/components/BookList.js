...
const BookList = () => {
  ...
  const fetchBooks = async () => {
    try {
      const response = await axios.get('https://pratyasha1.pythonanywhere.com/api/books/');
  ...
  const handleIssuing = async () => {
    try {
      const response = await axios.post('https://pratyasha1.pythonanywhere.com/api/issue-book/', {
  ...
  const handleDeleteBook = async (bookId) => {
    try {
      await axios.delete(`https://pratyasha1.pythonanywhere.com/api/books/${bookId}/`);
      fetchBooks(); // Refresh book list
    } catch (error) {
  ...
  return (
    ...
    <div className="p-4">
      ...
      <div className="mb-4 flex justify-center items-center">
        ...
        />
        <button
          className="bg-green-700 text-white py-2 px-4 rounded hover:bg-yellow-900 ml-2"
  ...
  {books.map(book => (
    <div key={book.id} className="bg-black rounded-lg shadow-md p-6 mb-4 text-white">
      ...
      <div className="flex justify-between items-center mt-4">
        <button
          className="bg-pink-700 text-white py-2 px-4 rounded hover:bg-blue-900"
